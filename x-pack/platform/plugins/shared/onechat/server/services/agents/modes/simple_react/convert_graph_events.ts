  ...
  return (streamEvents$) => {
    ...
    return streamEvents$.pipe(
      mergeMap((event) => {
        ...
        // stream answering text chunks for the UI (from the agent node)
        if (matchEvent(event, 'on_chat_model_stream') && matchName(event, 'agent')) {
        ...
        // emit tool calls when agent decides to use tools
        if (matchEvent(event, 'on_chain_end') && matchName(event, 'agent')) {
          const events: ConvertedEvents[] = [];
          const output = event.data.output;
          
          // The prebuilt React agent returns messages in the output
