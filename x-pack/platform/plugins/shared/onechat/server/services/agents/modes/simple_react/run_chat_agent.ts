...
const chatAgentGraphName = 'simple-react-onechat-agent';
...
/**
 * Create the handler function for the simple_react onechat agent.
 */
export const runSimpleReactAgentMode: RunChatAgentFn = async (
  {
    nextInput,
    conversation,
    agentConfiguration,
  ...
  // Prepend custom instructions as system prompt if provided
  const messagesWithSystemPrompt = resolvedConfiguration.research.instructions
    ? [
        {
          role: 'system' as const,
          content: resolvedConfiguration.research.instructions,
        },
        ...initialMessages,
      ]
    : initialMessages;
